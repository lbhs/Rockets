...
public class JewelMover : MonoBehaviour
{
    ...
    public void MovingJewel(int BondEnergy)  //BondEnergy value is passed from BondMaker Script
    {
        ...
        DisplayCanvasScript.PotentialEnergyInScene -= BondEnergy;
    }
    ...
    private IEnumerator MoveJewel(int BondEnergy)
    {
        ...
        {
            JewelsInMotion = true;
            transform.position = Vector3.MoveTowards(transform.position, targetForMovingJewels, 24*Time.deltaTime);   //third value is the speed at which Jewel moves
        ...
        if(TutorialSpeechBubbleScript.TutorialMessageNumber == 20)
        {
