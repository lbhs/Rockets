...
public class DragFlameTextVisibility : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        /*
        if (DieScript.totalRolls > 1 && DieScript.totalRolls < 5 && DisplayCanvasScript.JoulesOfHeat >0 && GameObject.Find("TutorialMarker").GetComponent<TutorialScript>().Tutorial == false)
        {
            DisplayDragText();
        }
        else*/
