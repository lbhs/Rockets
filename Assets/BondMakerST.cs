...
public class BondMakerST : MonoBehaviour
{
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (collision.collider.tag == "BondingTriggerDB" && collision.otherCollider.tag == "BondingTriggerDB")
        {
            ...
            if (BondEnergyMatrix.GetComponent<BondingCooldownScript>().BondingCooldownTimer == 0)
            {
    ...
    void OnTriggerEnter2D(Collider2D collider)  //triggered by an object colliding with the CircleCollider2D named "Bonding Trigger".  "collider" is the Other gameObject's collider that triggered this Collider2D
    {
        ...
        if (collider.tag == "InternalBondingSite" && BondEnergyMatrix.GetComponent<BondingCooldownScript>().BondingCooldownTimer > 0)  //this function makes a second bond between two multivalent atoms that have just initiated bonding
        {
        ...
        if (collider.tag == "InternalBondingSite" && BondEnergyMatrix.GetComponent<BondingCooldownScript>().BondingCooldownTimer == 0)    //Cooldown timer is set to 20 when a bonding event is initiated.  20 fixed updates will take it back to zero
        {
            VectorOffset = collider.GetComponent<VectorOffset>().BondingOffsetVector;
            BondingPartner = collider.transform.root.gameObject;  //add bonding partner to array of molecule's atoms
            BondingPartnerMoleculeID = BondingPartner.GetComponent<BondMaker>().MoleculeID;   //see what the Bonding partner's MoleculeID is
    ...
    public void MakeTheBonds()
    {
        ...
        if (TutorialSpeechBubbleScript.TutorialMessageNumber == 5 || TutorialSpeechBubbleScript.TutorialMessageNumber == 9 || TutorialSpeechBubbleScript.TutorialMessageNumber == 20 || TutorialSpeechBubbleScript.TutorialMessageNumber == 21 || TutorialSpeechBubbleScript.TutorialMessageNumber == 24)
        {
